...
class Embed(nn.Module):
    def __init__(self, attn_head=4, output_dim=128, d_k=64, d_v=64, attn_layers=4, dropout=0.1, disw=1.5, device='cuda:0'):
        ...
        self.gnns = nn.ModuleList([GraphConv(68, output_dim) if i == 0 else GraphConv(output_dim, output_dim) for i in range(attn_layers)])
...
class Fox(nn.Module):
    def __init__(self, task='clas', tasks=1, attn_head=4, output_dim=128, d_k=64, d_v=64, attn_layers=8, D=16, dropout=0.1, disw=1.5, device='cuda:0'):
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask, matrix):
        ...
        context, self.score = self.sdpa(Q, K, V, attn_mask, matrix)
